...
public class Zombie : Enemy
{
    ...
    public float hpRecoveryAmount; //체력 회복량
    ...
    {
        if (currentHP <= 0f || !enableMove) return; //HP가 모두 소진되거나 움직일 수 없으면 회복 금지

        float resultHP = currentHP + hpRecoveryAmount * Time.deltaTime; //결과 HP
