...
public class Skeleton : Enemy
{
    ...
    public SpriteRenderer hpBarSpriteRenderer; //HPBar의 Sprite Renderer 컴포넌트
    ...
    private bool isRunaway; //폭주 중인지 판단하는 변수
    public float runawayTime; //폭주 시간
    public float runawayMoveSpeed; //폭주 이동속도
    public float runawayConversionTime; //폭주 변환 시간
    public float runawaySetPlayerAfterImageMinDelay; //폭주 상태일 때 Player 잔상을 지정하는 최소 지연 시간
    public float runawaySetPlayerAfterImageMaxDelay; //폭주 상태일 때 Player 잔상을 지정하는 최대 지연 시간
    ...
    public float runawayRunAnimationSpeedMultiplier; //폭주 Run 애니메이션 속도 곱셈기
    public float runawayAttackAnimationSpeedMultiplier; //폭주 Attack 애니메이션 속도 곱셈기
    ...
    private readonly Color32 runawayHPBarColor = new Color32(80, 80, 80, 255); //폭주 HP Bar 색상
    ...
    private IEnumerator Runaway()
    {
        ...
        hpBarSpriteRenderer.color = runawayHPBarColor; //HP Bar 색상 변경
        moveSpeed = runawayMoveSpeed; //폭주 이동 속도 적용
        setPlayerAfterImageMinDelay = runawaySetPlayerAfterImageMinDelay; //잔상 생성 최소 속도 변경
        setPlayerAfterImageMaxDelay = runawaySetPlayerAfterImageMaxDelay;  //잔상 생성 최대 속도 변경

        while (currentHP < maxHP) //최대 체력에 도달하지 않으면
        {
            currentHP += Time.deltaTime * maxHP / runawayConversionTime; //체력 빠르게 증가
            currentHP = Mathf.Clamp(currentHP, 0f, maxHP); //체력 범위 수정

            yield return null;
        }

        spritesAnimator.Play(run_Hash); //Run 애니메이션 실행
        enableMove = true; //이동 허용
        enemyCollider2D.enabled = true; //Collider2D 활성화

        while (currentHP > 0f) //체력이 모두 사라지지 않으면
        {
            currentHP -= Time.deltaTime * maxHP / runawayTime; //체력 빠르게 감소
            currentHP = Mathf.Clamp(currentHP, 0f, maxHP); //체력 범위 수정

            yield return null;
        }

        spritesAnimator.Play(die_Hash); //Die 애니메이션 실행
    }
    ...
    private void ControlAnimatorParameter()
    {
        ...
        if (isRunaway) //폭주 중이면
        {
            spritesAnimator.SetFloat(run_Speed_Hash, runawayRunAnimationSpeedMultiplier * (1f - slowRate)); //Run 애니메이션 속도 지정
            spritesAnimator.SetFloat(attack_Speed_Hash, runawayAttackAnimationSpeedMultiplier * (1f - slowRate)); //Attack 애니메이션 속도 지정
