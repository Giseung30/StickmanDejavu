...
## ✔ 후기
+ 스마트폰이 보급된 지 얼마 지나지 않았을 때, 재밌게 즐겼던 **카툰워즈 거너** 게임을 오마주했다.
+ **상업적** 게임을 개발하는 방법을 정리할 수 있어서 큰 수확이라고 생각한다.
+ 평소에는 3D 위주의 프로젝트를 진행했으나, 이번에 **2D 프로젝트**를 진행하면서 **레이어** 개념을 완전히 익힐 수 있었다.
+ 1인 개발이었기에 **유닛 테스트**를 자주 진행하면서 디테일을 살리지 못했던 것이 아쉬운 부분이다.
+ 시간이 나면 멀티플레이가 가능하도록 개발해야겠다.
